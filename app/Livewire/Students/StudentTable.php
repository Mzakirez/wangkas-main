...
class StudentTable extends Component
{
    ...
    public Collection $schoolPrograms;
    ...
    public function mount(): void
    {
        ...
        $this->schoolPrograms = SchoolProgram::all();
    }
    ...
    public function render(): View
    {
        $students = Student::query()
            ...
            ->when($this->filters['schoolProgramID'], function (Builder $query) {
                return $query->where('school_program_id', '=', $this->filters['schoolProgramID']);
            })
            ->when($this->filters['gender'], function (Builder $query) {
                return $query->where('gender', $this->filters['gender']);
            })
            ->search($this->query)
            ->with(['schoolClass', 'schoolMajor', 'schoolProgram'])
