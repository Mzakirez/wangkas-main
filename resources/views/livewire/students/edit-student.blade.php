        ...
        <div class="modal-body">
          <form wire:submit="edit">
            ...
            <div class="row">
              ...
              <div class="mb-3">
                <x-forms.select-with-icon icon="bi-journal-bookmark" label="Program SPP" name="school_program_id"  wire:model="form.school_program_id" id="school_program_id">
                  @foreach ($school_programs as $program)
                    <option value="{{ $program->id }}">
                      {{ $program->name }} (Rp {{ number_format($program->fee, 0, ',', '.') }})
                    </option>
                  @endforeach
                </x-forms.select-with-icon>
                @error('form.school_program_id')
